...
class MLP(IModel):
    def __init__(self, hparams):
        ...
        self.fc1_bn = nn.BatchNorm1d(5 * hparams.data["input_size"])
        # Fully connected layer 2
        self.fc2 = nn.Linear(
            5 * hparams.data["input_size"], 25 * hparams.data["output_size"]
        )
        self.fc2_bn = nn.BatchNorm1d(25 * hparams.data["output_size"])
        # Fully connected layer 3
        self.fc3 = nn.Linear(
            25 * hparams.data["output_size"], 25 * hparams.data["output_size"]
        )
        self.fc3_bn = nn.BatchNorm1d(25 * hparams.data["output_size"])
        # Fully connected layer 4
        self.fc4 = nn.Linear(
            25 * hparams.data["output_size"], 5 * hparams.data["output_size"]
        )
        self.fc4_bn = nn.BatchNorm1d(5 * hparams.data["output_size"])
        # Fully connected layer 5
        self.fc5 = nn.Linear(
            5 * hparams.data["output_size"], hparams.data["output_size"]
        )
        self.fc5_bn = nn.BatchNorm1d(hparams.data["output_size"])
