...
class Transformer_Min_Max():
    ...
    def fit_transform(self, df:pd.DataFrame):
        ...
        self.numeric_columns = df.select_dtypes(include=["number"]).columns #get numeric columns
        ...
        # min-max scale
        df_copy[numeric_data.columns] = (numeric_data - self.mins) / (self.maxs - self.mins)
        # scale to feature range
        df_copy[numeric_data.columns] = (df_copy[numeric_data.columns] * (self.feature_range_max - self.feature_range_min) + self.feature_range_min)
...
class Transformer_PCA():
    ...
    def fit_transform(self, df:pd.DataFrame):
        ...
        self.numeric_columns = df.select_dtypes(include=["number"]).columns #get numeric columns
        self.non_numeric_columns = df.select_dtypes(exclude=["number"]).columns #get non numeric columns

        self.pca_converter = PCA(n_components=self.n_components) #init converter with n_components

        transformed_data = self.pca_converter.fit_transform(df[self.numeric_columns].to_numpy()) #fit and transform on numeric data

        df_transformed = df[self.non_numeric_columns].copy()

        df_transformed[[f"c_{i}" for i in range(transformed_data.shape[1])]] = transformed_data #append components as columns with prefix 'c_'
    ...
    def transform(self, df:pd.DataFrame):
        ...
        transformed_data = self.pca_converter.transform(df[self.numeric_columns].to_numpy()) #transform on numeric data
