...
def butt_highpass(df:pd.DataFrame, cut_f:float, order=3):
    """ Apply highpass (let trough frequencies above cut_f) filter on dataframe
    Args:
    ...
    b, a = butter(order, cut_f/nyq, fs=fs, btype='high') #create a highpass filter
...
with open(input_filename, "rb") as fr: #load data
    ...
    for key, segments in load(fr).items():
        data[key] = [butt_highpass(segment, cutoff, order) for segment in segments]
