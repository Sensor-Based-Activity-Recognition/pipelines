...
def read_files():
    ...
    df_train_test_split = pd.read_json(input_filename_traintest_split_file, typ="series")
...
if transformation_type == "min-max":
    ...
    segments, train_test_split = read_files() #read segment dictionary and train_test_split segment_ids

    structure = get_structure_dict(segments) #get structure of segments dict

    train_df, test_df = combine_df(segments, train_test_split["train"], train_test_split["test"]) #split segments into two separate dataframes containing train and test data
    ...
    segments_transformed = reconstruct_segments_dict(structure, train_df_transformed, test_df_transformed, train_test_split["train"], train_test_split["test"]) #store transformed segments

    write_files(segments_transformed, scaler) #write files

elif transformation_type == "standardize":
    print(
        f"Applying standardization"
    )
    ...
    print(
        f"Applying pca with n_components:{n_components}"
    )
